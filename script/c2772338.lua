...
    -- Always treated as a "Sky Striker Ace - Roze" card
    local e0=Effect.CreateEffect(c)
    e0:SetType(EFFECT_TYPE_SINGLE)
    e0:SetProperty(EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE)
    e0:SetCode(EFFECT_CHANGE_CODE)
    e0:SetValue(CARD_ROZE)
    c:RegisterEffect(e0)
...
    return c:IsSetCard(SET_SKY_STRIKER) and c:IsType(TYPE_MONSTER)
...
        local disg=Duel.GetMatchingGroup(Card.IsNegatable,tp,0,LOCATION_ONFIELD,nil)
...
    local is_ss_spell = c:IsSetCard(SET_SKY_STRIKER) and c:IsType(TYPE_SPELL)
    local is_rum_spell = c:IsSetCard(SET_RANK_UP_MAGIC) and c:IsType(TYPE_SPELL)
