...
function s.initial_effect(c)
    ...
    e0:SetType(EFFECT_TYPE)+EFFECT_FIELD
    ...
    -- HIỆU ỨNG 1 (Quick Effect): Tách 2 nguyên liệu; vô hiệu hóa hiệu ứng, sau đó nếu có 3+ Phép trong Mộ, úp sấp trục xuất tất cả các lá bài khác trên sân
    ...
    -- HIỆU ỨNG 2: Nếu thẻ bài của đối thủ bị trục xuất úp sấp bởi hiệu ứng bài bạn -> Thêm 1 Phép "Sky Striker" từ Deck, Mộ hoặc vùng trục xuất lên tay
    ...
    -- HIỆU ỨNG 3: Nếu thẻ bài khác bạn điều khiển rời sân do hiệu ứng của đối thủ -> Chọn 1 bài "Sky Striker" từ Mộ hoặc vùng trục xuất gắn vào bài này làm nguyên liệu
    ...
    -- HIỆU ỨNG 4: Nếu lá Xyz Summon này bị tiêu diệt bởi bài đối thủ -> Đặc biệt triệu hồi 1 quái "Sky Striker" từ Mộ/vùng trục xuất, rồi thêm 1 Phép "Sky Striker" từ Mộ/vùng trục xuất lên tay
...
function s.rumfilter(c,e,tp,xyzc)
    return c:IsCode(511000000) -- Đổi mã RUM Sky Striker tương ứng nếu có, hoặc check SetCard
...
function s.thcon(e,tp,eg,ep,ev,re,r,rp)
    return rp~=tp and eg:IsExists(function(c) return c:IsReason(REASON_EFFECT) and c:IsFacedown() and c:IsLocation(LOCATION_REMOVED) end,1,nil)
end
...
function s.matfilter(c,tp)
    return c:IsControler(tp) and c:IsPreviousLocation(LOCATION_FIELD) and c~=e:GetHandler() and c:IsReason(REASON_EFFECT) and c:GetReasonPlayer()~=tp
end
...
function s.mattg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
    ...
    if chk==0 then return e:GetHandler():IsRelateToEffect(e) and Duel.IsExistingTarget(s.attfilter,tp,LOCATION_GRAVE+LOCATION_REMOVED,0,1,nil) end
...
function s.spcon(e,tp,eg,ep,ev,re,r,rp)
    local c=e:GetHandler()
    return c:IsSummonType(SUMMON_TYPE_XYZ) and rp~=tp and c:IsReason(REASON_DESTROY)
end
