-- Chrysos Heirs: Anaxagoras - Demigod of Rift
-- ID: 888800010
local s,id=GetID()
...
function s.initial_effect(c)
	...
	-- MONSTER EFFECTS
	----------------------------------------------------------------------------
	-- Eff 1: Khi được Triệu hồi Đặc biệt: Hiến tế 1 quái "Chrysos Heirs" từ Tay hoặc Deck (1 lần/lượt)
...
-- EFFECT 1 LOGIC
--------------------------------------------------------------------------------
function s.ribfilter(c)
	return c:IsSetCard(SET_CHRYSOS_HEIRS) and c:IsType(TYPE_MONSTER)
		and not c:IsCode(id) and (c:IsReleasableByEffect() or c:IsLocation(LOCATION_DECK))
end

function s.rbtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.ribfilter,tp,LOCATION_HAND+LOCATION_DECK,0,1,nil) end
...
function s.rbop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,s.ribfilter,tp,LOCATION_HAND+LOCATION_DECK,0,1,1,nil)
	if #g>0 then
...
function s.thspop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 and Duel.SendtoHand(g,nil,REASON_EFFECT)>0 and g:GetFirst():IsLocation(LOCATION_HAND) then
		...
		-- Bổ sung true (ignore summoning conditions) để lá này tự hồi sinh khi bị Tributed từ Hand/Deck
