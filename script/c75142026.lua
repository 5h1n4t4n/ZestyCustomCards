...
function s.nscost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.IsExistingMatchingCard(s.cfilter,tp,LOCATION_HAND+LOCATION_ONFIELD,0,1,c,c) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
	local g=Duel.SelectMatchingCard(tp,s.cfilter,tp,LOCATION_HAND+LOCATION_ONFIELD,0,1,1,c,c)
...
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 and Duel.SendtoHand(g,nil,REASON_EFFECT)>0 then
		...
		if Duel.IsExistingMatchingCard(s.warfilter,tp,LOCATION_HAND+LOCATION_MZONE,0,1,nil)
