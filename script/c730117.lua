...
function s.initial_effect(c)
	...
	e3:SetCategory(CATEGORY_DISABLE+CATEGORY_TOHAND+CATEGORY_SEARCH+CATEGORY_TODECK)
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		if is_opp_turn then
			--Activated as Chain Link 2 or higher during opponent's turn
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if is_opp_turn then
		--Negate the effects of all other cards in this Chain
		local current_chain=Duel.GetCurrentChain()
		for i=1,current_chain-1 do
			Duel.NegateEffect(i)
		end
	else
		--Your turn: Reveal top 10 cards, add 2 to hand, shuffle this card into Deck, cannot draw cards
