...
function s.thspop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 and Duel.SendtoDeck(g,nil,SEQ_DECKSHUFFLE,REASON_EFFECT)>0 then
		local b1=Duel.IsExistingMatchingCard(function(c) return c:IsSetCard(SET_MAVERICK_ANALYZER) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand() end,tp,LOCATION_DECK,0,1,nil)
		local b2=Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.IsExistingMatchingCard(function(c) return c:IsSetCard(SET_MAVERICK_ANALYZER) and c:IsType(TYPE_MONSTER) and c:IsCanBeSpecialSummoned(e,0,tp,false,false) end,tp,LOCATION_DECK,0,1,nil)
		...
		if opt==0 then
			...
			local sg=Duel.SelectMatchingCard(tp,function(c) return c:IsSetCard(SET_MAVERICK_ANALYZER) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand() end,tp,LOCATION_DECK,0,1,1,nil)
			if #sg>0 then
			...
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
			local sg=Duel.SelectMatchingCard(tp,function(c) return c:IsSetCard(SET_MAVERICK_ANALYZER) and c:IsType(TYPE_MONSTER) and c:IsCanBeSpecialSummoned(e,0,tp,false,false) end,tp,LOCATION_DECK,0,1,1,nil)
			if #sg>0 then
