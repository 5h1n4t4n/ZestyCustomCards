...
Theo lựa chọn của chủ repo, **toàn bộ `card-data/c*.json`** được biên dịch vào **`card-data.cdb`**. Tên file biểu thị nguồn dữ liệu, không suy đoán tác giả qua Git. Card không có spec vẫn nằm trong `custom_cards_zesty.cdb` hoặc `mycard.cdb`; compiler không ghi vào hai file đó.
...
Đóng game và các trình sửa CDB trước khi chạy migration; không chạy đồng thời với compiler. Ba file được thay lần lượt với cơ chế khôi phục khi lỗi, không phải một transaction chung.
...
Migration không phải thao tác thường ngày. Sau khi áp dụng, phân phối đồng bộ `card-data.cdb`, `custom_cards_zesty.cdb`, `mycard.cdb` để client không còn nạp cùng ID từ CDB cũ. Không suy đoán thứ tự nạp có thể giải quyết trùng ID.
...
- Trước merge/commit, kiểm tra ID mới chưa có trong cả ba CDB; không tự đổi ID hiện hữu vì có thể làm hỏng deck.
